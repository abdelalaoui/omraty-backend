...
final class ServiceCardTable {
    ...
    static final String SERVICE_CARD_COLUMNS =
            "id, type, title, description, button_text, icon, image_url, coming_soon, visible,"
                    + " updated_at";
    ...
    static final String INSERT_SERVICE_CARD =
            "INSERT INTO service_card (type, title, description, button_text, icon, image_url,"
                    + " coming_soon, visible) VALUES (?, ?, ?, ?, ?, ?, ?, ?) RETURNING "
                    + SERVICE_CARD_COLUMNS;
    ...
    static final String UPDATE_SERVICE_CARD =
            "UPDATE service_card SET type = COALESCE(?, type), title = COALESCE(?, title),"
                    ...
                    + " button_text), icon = COALESCE(?, icon), image_url = COALESCE(?, image_url),"
                    + " coming_soon = COALESCE(?, coming_soon), visible = COALESCE(?, visible),"
