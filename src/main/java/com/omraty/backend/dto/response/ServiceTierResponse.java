 ...
 * des lits avec capacity et price, VIP → parcours VIP, AGENCY → écran agence) ; ne jamais en
 * déduire le comportement à partir de label, purement éditorial. price remplace le montant codé en
 * dur côté app (RoomBedsScreen._mockAmount) ; nullable comme capacity le temps que l'admin le
 * saisisse pour les formules ROOM existantes. label est résolu côté backend à partir du header
 * Accept-Language de la requête (fr|en|ar, ar par défaut si absent/non reconnu — voir
